...
class TestConnectorWmsWhs(CommonConnectorWMS):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.dbsource = cls.env.ref("connector_whs.demo_wms_mssql")
        ...
        with unittest.TestCase().assertRaises(ValidationError):
            cls.dbsource.connection_test()
        cls.dbsource.with_context(no_return=True).execute_mssql(
            sqlquery=clean_sql_text("DELETE FROM HOST_LISTE"),
            sqlparams=None,
            metadata=None,
        )
        cls.categ_id = cls.env.ref("product.product_category_3")
        cls.custom_categ_id = cls.env["product.category"].search(
            [
                ("name", "=", "CUSTOM"),
            ]
        )
        if not cls.custom_categ_id:
            cls.custom_categ_id = cls.env["product.category"].create(
                {
                    "name": "CUSTOM",
                }
            )
    ...
    def _select_whs_liste_rif(self, riferimento):
        return self.dbsource.execute_mssql(
            sqlquery=clean_sql_text(
                "SELECT Elaborato FROM HOST_LISTE WHERE Riferimento like :Riferimento"
            ),
            sqlparams=dict(Riferimento=f"%{riferimento}%"),
            metadata=None,
        )
    ...
    def _check_cancel_workflow(self, picking, list_len):
        """
        This method is used to check the creation of new whs list linked to the
        picking when this is canceled, invalidating the existing ones.
        """
        riferimento = (
        ...
        whs_lists = picking.mapped("move_ids.whs_list_ids")
        self.assertEqual(len(whs_lists), list_len)
        self.assertEqual(set(whs_lists.mapped("stato")), {"2"})
        picking.action_assign()
        if all(x.state == "assigned" for x in picking.move_ids):
        ...
        self.assertEqual(set(picking.move_ids.mapped("whs_list_ids.stato")), {"3"})
        self.simulate_whs_cron(
            {x: x.qta for x in picking.move_ids.mapped("whs_list_ids")}, 5
        )
        riferimento = (
            picking.sale_id.name if picking.sale_id else picking.purchase_id.name
        )
        ...
        if picking.sale_id:
            picking.action_assign()
        valid_whs_lists = picking.mapped("move_ids.whs_list_ids").filtered(
            lambda x: x.stato != "3"
        )
        ...
        for Elaborato in {x[0] for x in whs_records2}:
            # WHS cron change Elaborato to 2 in an uncontrollable time
    ...
    def _execute_select_all_valid_host_liste(self):
        ...
        if res[0]:
            cols = res[1]
            rows_with_headers = [dict(zip(cols, row, strict=False)) for row in res[0]]
            return len(res[0]), rows_with_headers
        return 0, []
    ...
    def test_00_complete_picking_from_sale(self):
        ...
        whs_len_records, _whs_records = self._execute_select_all_valid_host_liste()
        ...
        self.assertEqual(len(picking1.mapped("move_ids.whs_list_ids")), 1)
        if all(x.state == "assigned" for x in picking1.move_ids):
            self.assertEqual(picking1.state, "assigned")
        else:
            self.assertEqual(picking1.state, "waiting")
        # check WMS list is added
        self.dbsource.whs_insert_read_and_synchronize_list()
        new_whs_len_records, whs_records = self._execute_select_all_valid_host_liste()
        self.assertEqual(new_whs_len_records, whs_len_records + 1)
        for whs_record in whs_records:
            client_order_ref = whs_record["AuxTesto1"]
            default_code = whs_record["Articolo"]
            product_code = whs_record["AuxTestoRiga1"]
            self.assertEqual(client_order_ref, order1.client_order_ref)
            self.assertEqual(self.product1.customer_ids[0].product_code, product_code)
            self.assertEqual(self.product1.default_code, default_code)
        # check cancel workflow
        whs_lists = picking1.mapped("move_ids.whs_list_ids")
        ...
        self.assertEqual(picking1.move_ids.mapped("whs_list_ids.stato"), ["3"])
        # restore picking to assigned state
        picking1.action_back_to_draft()
        picking1.action_confirm()
        picking1.action_assign()
        whs_lists = picking1.mapped("move_ids.whs_list_ids").filtered(
            lambda x: x.stato != "3"
        )
        self.assertEqual(len(whs_lists), 1)
        ...
        self.dbsource.whs_insert_read_and_synchronize_list()
        new_whs_len_records, _whs_records = self._execute_select_all_valid_host_liste()
        self.assertEqual(new_whs_len_records, whs_len_records + 1)
        # simulate whs work
        lotto = "55A1"
        lotto2 = "55A2"
    ...
    def test_01_partial_picking_from_sale(self):
        ...
        self.assertEqual(len(picking.mapped("move_ids.whs_list_ids")), 2)
        self.assertEqual(len(set(picking.mapped("move_ids.whs_list_ids.num_lista"))), 1)

        # check WMS list is added
        self.dbsource.whs_insert_read_and_synchronize_list()
        new_whs_len_records, whs_records = self._execute_select_all_valid_host_liste()
        self.assertEqual(new_whs_len_records, whs_len_records + 2)
        self.assertEqual({x["Elaborato"] for x in whs_records}, {1})
        self.assertEqual(
            {x.stato for x in picking.mapped("move_ids.whs_list_ids")}, {"2"}
        )
        for whs_record in whs_records:
            client_order_ref = whs_record["AuxTesto1"]
            default_code = whs_record["Articolo"]
        ...
        self.dbsource.whs_insert_read_and_synchronize_list()
        whs_lists = backorder_picking.mapped("move_ids.whs_list_ids").filtered(
            lambda x: x.stato != "3"
        )
        for whs_list in whs_lists:
        ...
        self.assertEqual(backorder_picking.move_ids[0].state, "assigned")
        self._delete_all_sql_records()
    ...
    def test_02_partial_picking_partial_available_from_sale(self):
        ...
        self.dbsource.whs_insert_read_and_synchronize_list()
        new_whs_len_records, whs_records = self._execute_select_all_valid_host_liste()
        elaborato_records = [x for x in whs_records if x["Elaborato"] != 5]
        self.assertEqual(len(elaborato_records), whs_len_records + 2)
        for whs_record in elaborato_records:
            client_order_ref = whs_record["AuxTesto1"]
            default_code = whs_record["Articolo"]
            product_code = whs_record["AuxTestoRiga1"]
            self.assertEqual(client_order_ref, order1.client_order_ref)
        ...
        for move in picking.move_ids:
            ...
            self.assertAlmostEqual(
                move.quantity, 3.0 if move.product_id == self.product1 else 20
            )
        picking.action_assign()
        self.assertEqual(picking.state, "assigned")
        ...
        self.assertEqual(len(order1.picking_ids), 2)
        backorder_picking = order1.picking_ids - picking
        back_whs_list = backorder_picking.mapped("move_ids.whs_list_ids")
        self.assertEqual(backorder_picking.move_ids.mapped("state"), ["assigned"])
    ...
    def test_03_partial_picking_from_sale(self):
        ...
        picking = order1.picking_ids[0]
        self.assertEqual(len(picking.mapped("move_ids.whs_list_ids")), 4)

        # check WMS list is added
        self.dbsource.whs_insert_read_and_synchronize_list()
        new_whs_len_records, whs_records = self._execute_select_all_valid_host_liste()
        self.assertEqual(new_whs_len_records, whs_len_records + 4)
        for whs_record in whs_records:
            client_order_ref = whs_record["AuxTesto1"]
            default_code = whs_record["Articolo"]
            product_code = whs_record["AuxTestoRiga1"]
        ...
        for move_line in picking.move_ids:
            for stock_move_line in move_line.move_line_ids:
                if stock_move_line.product_id in [
                    ...
                    self.assertAlmostEqual(stock_move_line.quantity, 5.0)
                if stock_move_line.product_id == self.product3:
                    self.assertAlmostEqual(stock_move_line.quantity, 0)
        self.run_stock_procurement_scheduler()
        # check that action_assign run by scheduler do not change state
        # self.assertEqual(picking.state, "confirmed")
        picking.action_assign()
        ...
        for move_line in picking.move_ids:
            self.assertAlmostEqual(
                move_line.whs_list_ids[0].qtamov,
                5
                if move_line.product_id in [self.product1, self.product2, self.product4]
                else 0,
            )
        ...
        self.assertEqual(len(order1.picking_ids), 2)
        backorder_picking = order1.picking_ids - picking
        for move_line in backorder_picking.move_ids:
            self.assertAlmostEqual(
                ...
                msg=(
                    f"Unexpected backorder WMS quantity for "
                    f"{move_line.product_id.display_name} "
                    f"(move {move_line.id}, lists {move_line.whs_list_ids.ids})"
                ),
            )
        ...
        new_whs_len_records, _whs_records = self._execute_select_all_valid_host_liste()
        self.assertEqual(new_whs_len_records, whs_len_records + 6)
        # self.run_stock_procurement_scheduler()
    ...
    def test_04_unlink_sale_order(self):
        ...
        wizard = Form.from_action(self.env, order1.action_cancel()).save()
        ...
        self.assertEqual(
            set(order1.mapped("picking_ids.move_ids.whs_list_ids.stato")), {"3"}
        )
        order1.action_draft()
        order1.action_confirm()
        picking = order1.picking_ids.filtered(lambda x: x.state != "cancel")
        self.assertEqual(picking.mapped("move_ids.whs_list_ids.stato"), ["1", "1"])
        # insert lists in WMS: this has to be invoked before every sql call!
        self.dbsource.whs_insert_read_and_synchronize_list()
        new_whs_len_records, _whs_records = self._execute_select_all_valid_host_liste()
        self.assertEqual(
        ...
        picking.action_assign()
        self.assertEqual(picking.state, "assigned")
        hyddemo_whs_lists = picking.mapped("move_ids.whs_list_ids")
        ...
        new_product_move_line_ids = order1.picking_ids.mapped("move_ids").filtered(
    ...
    def test_06_purchase(self):
        ...
        # - product 2: 20 units
        # - product 3: 3 units
        # then change manually qty of product 2 to 17 units on both PO and picking
        # then transfer 2 units of product 2 and all of product 3 (3 units)
        with self.assertRaises(ValidationError):
            self.dbsource.connection_test()
        ...
        move_line = purchase.picking_ids.move_ids.filtered(
        ...
        move_line.quantity = 17
        self.assertEqual(order_line.product_qty, move_line.whs_list_ids.qta)
        # check whs list is changed (was added until v.14.0)
        self.dbsource.whs_insert_read_and_synchronize_list()
        new_whs_len_records, _whs_records = self._execute_select_all_valid_host_liste()
        self.assertEqual(
            new_whs_len_records,
        ...
        qty_available_product2 = self.product2.with_context(
            location=self.dbsource.location_id.id
        ).qty_available
        # simulate WMS work: partial processing of product #2
        # and total of product #3
        whs_lists = purchase.mapped("picking_ids.move_ids.whs_list_ids")
        self.simulate_whs_cron(
        ...
        new_qty_available_product2 = self.product2.with_context(
            location=self.dbsource.location_id.id
        ).qty_available
        ...
        picking.action_assign()
        self.assertIn(picking.state, ["assigned", "waiting"])
        # check that action_assign run by scheduler do not change state
        picking.action_assign()
        self.assertEqual(picking.state, "assigned")
        wizard = Form.from_action(self.env, picking.button_validate()).save()
        ...
        # self.run_stock_procurement_scheduler()
        backorder_picking.action_assign()
        if all(x.state == "assigned" for x in backorder_picking.move_ids):
            self.assertEqual(backorder_picking.state, "assigned")
        ...
        backorder_picking.action_assign()
        # simulate WMS work set done to rest of backorder add 3 more units
        new_back_whs_list = backorder_picking.mapped("move_ids.whs_list_ids").filtered(
            lambda x: x.stato != "3"
        )
        self.simulate_whs_cron({x: 18 for x in new_back_whs_list}, 4)
        ...
        self.assertFalse(
            all(
                whs_list.stato == "3"
                for whs_list in backorder_picking.move_ids.whs_list_ids
            )
        )
        ...
        new_product_move_line_ids = purchase.picking_ids.mapped("move_ids").filtered(
    ...
    def test_07_1_purchase_no_backorder_with_less_qty(self):
        ...
        move_line = purchase.picking_ids.move_ids
        self.assertEqual(order_line.product_qty, move_line.whs_list_ids.qta)
        self.dbsource.whs_insert_read_and_synchronize_list()
        new_whs_len_records, _whs_records = self._execute_select_all_valid_host_liste()
        self.assertEqual(
        ...
        whs_list = purchase.mapped("picking_ids.move_ids.whs_list_ids")
    ...
    def _mrp_partial_from_sale(self, man_orders, is_custom=False):
        ...
        # this update Odoo from WHS
        self.assertEqual(man_order.qty_produced, 0)
        self.dbsource.whs_insert_read_and_synchronize_list()
        self.assertEqual(man_order.state, "progress")
        self.assertTrue(man_order.sent_to_whs)
        man_order.button_consume()
        self.assertTrue(man_order.sent_to_whs)
        self.assertEqual(man_order.state, "consumed")
        self.assertEqual(man_order.qty_produced, 0)
        action = man_order.with_context(test_connector_whs=True).button_mark_done()
        # there is no consumption warning as we used the exact component quantities to
        # produce 25% of the products
        self.assertEqual(
        ...
        with self.assertRaises(UserError):
            # check production backorder cannot be done without WHS lists
    ...
    def test_10_sync_inventory_wizard(self):
        ...
        # Current qty in WMS base location and its children
        ...
        line = log.hyddemo_mssql_log_line_ids.filtered(
            lambda hyl: hyl.name == product.default_code
        )
        ...
        warehouse = base_loc.warehouse_id
    ...
    def test_11_product_master_roundtrip_no_loop(self):
        ...
        prod_form.type = "consu"
        prod_form.is_storable = True

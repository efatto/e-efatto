...
class BaseTestCase(common.TransactionCase):
    ...
    def run_import_with_amount(self, file_name, file_type):
        ...
        with open(os.path.join(_path, 'tests/data', file_name), 'rb') as ofile:
            import_wizard = self.env[
                'account.bank.statement.import.ex.wizard'].create({
                    ...
                    'file_name': file_name,
                    'file_type': file_type})
            import_wizard.with_context(journal_id=self.bank_journal.id).do(
                fields, self.options)
            statement = self.env['account.bank.statement'].search([
                ('reference', '=', file_name)], limit=1)
            ...
            line = statement.line_ids.filtered(
                lambda rec: rec.name == 'STATEMENT 10')
            self.assertEqual(line.date, datetime.date(2018, 1, 8).strftime(
    ...
    def run_import_with_partner(self, file_name, file_type):
        ...
        with open(os.path.join(_path, 'tests/data', file_name), 'rb') as ofile:
            ...
            statement = self.env['account.bank.statement'].search([
                ('reference', '=', file_name)], limit=1)
            line = statement.line_ids.filtered(
                lambda rec: rec.name == 'STATEMENT 15')

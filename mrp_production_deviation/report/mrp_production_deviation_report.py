    ...
    def init(self):
        ...
        cost_expected = workorder.duration_expected * workcenter.costs_hour / 60 +
            coalesce(MAX(bomline.product_qty * production.product_qty
                * ABS(stockmove.price_unit)), 0) => costo previsto inizialmente sulla
                base del tempo di lavorazione iniziale salvato nel workorder + costo
                delle righe della bom moltiplicato per i pezzi da produrre
        cost_expected_rw = routing.time_cycle_manual * production.product_qty
                * workcenter.costs_hour / 60 +
            coalesce(MAX(bomline.product_qty * production.product_qty
                * ABS(stockmove.price_unit)), 0) => costo attuale del tempo di
                 lavorazione calcolato dal routing moltiplicato per i pezzi da produrre
                 + resto come sopra

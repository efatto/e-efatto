...
class AccountMove(models.Model):
    ...
    def _compute_amount_total_without_delivery(self):
        self.ensure_one()
        delivery_cost = sum([
            l.price_subtotal for l in self.invoice_line_ids if l.is_delivery])
    ...
    def rate_shipment(self, carrier):
        ...
        if hasattr(carrier, '%s_rate_shipment' % carrier.delivery_type):
            ...
            if (
                res['success'] and carrier.free_over
    ...
    def _create_delivery_line(self, carrier, price_unit):
        ...
        if carrier.free_over and self.currency_id.is_zero(price_unit):
            values['name'] += '\n' + _('Free Shipping')
        delivery_line = self.env['account.move.line'].sudo().with_context(
            check_move_validity=False
        ).create(values)
        del context
        discount = self.env.context.get("delivery_discount")  # fixme non si può mettere

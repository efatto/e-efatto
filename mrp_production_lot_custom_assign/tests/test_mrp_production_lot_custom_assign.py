...
class TestMrpProductionLotCustomAssign(TestProductionData):
    ...
    def setUpClass(cls):
        ...
        cls.routing = cls.env["mrp.routing"].create(
            {
                "name": "Operation in workcenter",
                "operation_ids": [
                    (
                        6,
                        0,
                        cls.routing_tmpl_1.ids,
                    ),
                ],
            }
        )
    ...
    def _create_production(self, tracking):
        ...
        if tracking == "serial":
            self.assertEqual(man_order.product_qty, 1)
        ...
        if tracking != "none":
            ...
            if tracking == "lot":
                ...
                i = 0
                for finished_move_line in man_order.finished_move_line_ids:
                    finished_move_line.lot_id = self.env["stock.production.lot"].search(
                        [("name", "=", "Final lot %s %s" % (i, tracking))],
                    )
                    finished_move_line._onchange_serial_number()
                    i += 1
            man_order.button_plan()
            self.assertEqual(
                man_order.workorder_ids[0].finished_lot_id,
            ...
            workorder.finished_lot_id = other_lot_id
            with self.assertRaises(UserError):
                workorder.sudo(self.mrp_user).button_done()
            workorder.finished_lot_id = first_final_lot
            workorder.sudo(self.mrp_user).button_done()
            # test progressive final lot for workorders for serial
            if tracking == "serial":
                self.assertIn(
                    workorder.finished_lot_id,
                    man_order.mapped("finished_move_line_ids.lot_id"),
                )
                for _qty in range(0, int(workorder.qty_remaining)):
                    workorder.sudo(self.mrp_user).button_done()

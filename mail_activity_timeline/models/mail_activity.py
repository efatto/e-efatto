...
class MailActivity(models.Model):
        ...
        for activity in self:
            if activity.is_resource_planner:
                if any(
                    ...
                    res_object.with_context(bypass_resource_planner=True).write(vals)
                    if "name" in vals:
    ...
    def create_planner_activity(self, res_object, user_id):
        res_model = self.env["ir.model"].search([("model", "=", res_object._name)])
        ...
        vals = {
            ...
            if res_object._name == "mrp.workorder"
                ...
                "mail_activity_timeline.mail_activity_project_mrp_workcenter"
            ).id,
    ...
    def _compute_planner(self):
        ...
        for activity in self:
            ...
            if (
                activity.res_model
                and activity.res_id
                and activity.res_model in ["mrp.workorder", "project.task"]
            ):
                res_object = self.env[activity.res_model].browse(activity.res_id)
                if activity.res_model == "mrp.workorder":
                    vals.update(
                        ...
                        vals.update(
                            {
                                "color_active": self.get_color(
                                    activity, res_object.project_id.activity_color
                                ),
                            }
                        )
                    else:

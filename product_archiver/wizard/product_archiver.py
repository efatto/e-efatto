...
class ProductArchiver(models.TransientModel):
    ...
    def archive(self):
        ...
        to_unarchive_product_tmpls = (
            self.env["product.product"]
            .search([])
            .mapped("product_tmpl_id")
            .filtered(lambda x: not x.active)
        )
        ...
        ).filtered(lambda x: not x.qty_available and not x.virtual_available)
        # search moved product after from_date to exclude them
        # from stock.move.line
        stock_move_line_products = (

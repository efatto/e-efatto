...
class ProductProduct(models.Model):
    ...
    last_purchase_price = fields.Float(related="last_purchase_line_id.price_unit")
    last_purchase_discount = fields.Float(related="last_purchase_line_id.discount")
    last_purchase_discount2 = fields.Float(related="last_purchase_line_id.discount2")
    last_purchase_discount3 = fields.Float(related="last_purchase_line_id.discount3")
    last_purchase_date = fields.Datetime(related="last_purchase_id.date_order")
    ...
    last_supplier_invoice_price = fields.Float(
        related="last_supplier_invoice_line_id.price_unit", string="Invoice Unit Price"
    )
    last_supplier_invoice_discount = fields.Float(
        related="last_supplier_invoice_line_id.discount", string="Invoice Discount (%)"
    )
    ...
    def set_product_last_supplier_invoice(self, invoice_id=False):
        ...
        for product in self:
            ...
            product.write(
                {
                    "last_supplier_invoice_line_id": last_line.id
                    if last_line
                    else False,
                }
            )

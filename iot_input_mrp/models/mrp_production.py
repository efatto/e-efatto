...
class MrpWorkorder(models.Model):
    ...
    def button_start(self):
        ...
        if res and new_self.workcenter_id.iot_device_input_id:
            self.production_id.mrp_start_produce(new_self.workcenter_id)
        return res
    ...
    def record_production(self):
        ...
        if res and new_self.workcenter_id.iot_device_input_id:
            duration = self.production_id.mrp_end_produce(new_self.workcenter_id)
            if duration:
                timesheet = self.time_ids[0]
                timesheet.date_end = timesheet.date_start + timedelta(seconds=duration)
        return res
...
class MrpProduction(models.Model):
    ...
    weight = fields.Float(copy=False, digits=dp.get_precision("Stock Weight"))
    ...
    def button_plan(self):
        for production in self.filtered(lambda x: x.workcenter_ids):
            if production.workcenter_ids[0].is_busy:
                raise ValidationError(
                    _("Production cannot be started as workcenter " "is busy!")
                )
        ...
        for production in self.filtered(lambda x: x.workcenter_ids):
            self.env.cr.execute(
                ...
                % (
                    production.sudo()
                    .workcenter_ids[0]
                    .iot_device_input_id.device_id.device_identification,
    ...
    def align_weight(self, weight):
        ...
        total_weight = sum(
            x.product_uom._compute_quantity(
                x.product_uom_qty, reference_weight_uom_id, round=False
            )
            for x in moves
        )
        if total_weight:

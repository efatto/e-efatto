...
class QualityControlStockOcaFailed(BaseCommon):
    ...
    def test_00_purchase_order(self):
        ...
        move_with_qc_ins = picking.move_ids.filtered(
            lambda x: x.product_id == self.product2
        )
        move_without_qc_ins = picking.move_ids - move_with_qc_ins
        self.assertEqual(move_with_qc_ins, picking.qc_inspections_ids.object_id)
        self.assertEqual(
            move_with_qc_ins.move_line_ids.mapped("location_dest_id"),
            picking.picking_type_id.warehouse_id.wh_qc_stock_loc_id,
        )
        self.assertEqual(
            move_without_qc_ins.move_line_ids.mapped("location_dest_id"),

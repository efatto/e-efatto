...
class MrpRouting(models.Model):
    ...
    def _compute_estimated_duration(self):
        for routing in self:
            routing.estimated_days_duration = sum(
                routing.operation_ids.mapped("estimated_days_duration")
            )
            routing.estimated_duration = sum(
                routing.operation_ids.mapped("estimated_duration")
            )

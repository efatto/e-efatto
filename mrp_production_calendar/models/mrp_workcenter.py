    ...
    def _compute_exceeded_capacity(self):
        for workcenter in self:
            planned_wo_ids = workcenter.order_ids.filtered(
                ...
                and x.date_planned_start and x.date_planned_finished
            )
            ...
            workcenter.wo_exceeded_capacity_value = (
                0 if not (exceeded_capacity_wo_ids or planned_wo_ids) else (
                    len(exceeded_capacity_wo_ids) / len(planned_wo_ids)
                )
            )
